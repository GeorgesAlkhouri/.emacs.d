...
Download top stories from  [[https://news.ycombinator.com][Hacker News]].
...
#+begin_src emacs-lisp
  ...
  ;; TODO speed up function by executing all requests simultaneously
  (defun hackernews-get-items (list-of-ids completed-items callback)
    "Asynchronously request LIST-OF-IDS from hackernews.  After requests are completed call CALLBACK with COMPLETED-ITEMS."
    (if (not list-of-ids)
	(funcall callback completed-items)
      (let* ((request-string (format "%s/%s/item/%s.json" hackernews-api-base hackernews-api-version (pop list-of-ids)))
	     (result (request request-string
			      ...
			      :success  (cl-function
					 (lambda (&key data &allow-other-keys)
					   (hackernews-get-items list-of-ids (append completed-items (list data)) callback))))))
	)
      )
    )
