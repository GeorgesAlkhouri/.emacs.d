...
#+begin_src emacs-lisp
  ...
  (cond (*sys/mac*
	 ...
	 (defconst *sys/omnisharp-server-path* "/usr/local/share/omnisharp-osx/run")
	 )
	(*sys/linux*
	 (defconst *sys/font-default-height* 105)
	 (defconst *sys/shell-executable* "/bin/bash")
	 (defconst *sys/omnisharp-server-path* nil)
	 ))
...
Download OmniSharp-Roslyn from https://github.com/OmniSharp/omnisharp-roslyn/releases and tell lsp-mode where to find the =lsp-csharp-server-path=.
